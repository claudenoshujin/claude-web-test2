...
3.0 的档案版式是独立实现，不再拼接 2.0 的档案 CSS。旧版完整快照保存在
`legacy/2.0/`，不要把那一层当成扩展根目录安装。
...
```
https://github.com/claudenoshujin/claude-web-test
```

装完的文件夹名等于仓库名，所以这个仓库必须叫 `claude-web-test`。
...
把这个目录整个放进
`SillyTavern/public/scripts/extensions/third-party/claude-web-test/`，刷新页面。
...
通用交互源码在开发仓库的 `src/`，3.0 档案视觉在 `src-v3/`。
改完这样重新出包：

```powershell
cd "E:\Crab Pot\酒馆\Claude-Clawd-酒馆助手"
$env:CLAUDE_EXTENSION_FLAT="1"
$env:CLAUDE_EXTENSION_OUTPUT_DIR="E:\Crab Pot\酒馆\claude-web-test"
node tools/build-extension.js
node tools/verify-extension.js

cd "E:\Crab Pot\酒馆\claude-web-test"
git add -A
